import { MonthStorage } from '@/types/record';
...
const calendar = {
  ...
  getMonthStorage: (year: number, month: number) => {
    ...
    const firstDateCurrentMonth = calendar.getFirstDateCurrentMonth(
      year,
      month
    );
    ...
    const currentMonthStorage = calendar.getDatesCurrentMonth(
  ...
  // 이전달의 마지막 날
  getLastDatePrevMonth: (year: number, month: number) =>
    new Date(year, month - 1, 0),

  // 다음달의 첫번째 날
  getFirstDateNextMonth: (year: number, month: number) =>
    new Date(year, month, 1),

  // 이번달의 첫번째 날
  getFirstDateCurrentMonth: (year: number, month: number) =>
    new Date(year, month - 1),

  // 이번달의 마지막 날
  getLastDateCurrentMonth: (year: number, month: number) =>

    ...
    <ToolTipWrapper>
      ...
      {!showBubbleOnly && (
        <ToolTipContainer type="button" onClick={onClickToolTip}>
          <Image src={ToolTipButton} alt="tooltip" />
        </ToolTipContainer>
      )}
      {showBubble && (
        <BubbleWrapper
...
interface BubbleWrapperProps
  extends StyledProps<
    Pick<ToolTipProps, 'width' | 'direction' | 'left' | 'edgeLeft'>
  > {}
...
const BubbleWrapper = styled.div<BubbleWrapperProps>`
  ...
  z-index: 2;
  ${({ $left }) =>
    $left ? `left: ${$left};` : 'left: 50%; transform: translateX(-50%);'}
  ...
  &::before {
    ...
    border-top: 15px solid
      ${({ $direction, theme }) =>
        $direction === 'top' ? theme.color.grey800 : 'transparent'};
    border-right: 10px solid transparent;
    border-bottom: 15px solid
      ${({ $direction, theme }) =>
        $direction === 'bottom' ? theme.color.grey800 : 'transparent'};

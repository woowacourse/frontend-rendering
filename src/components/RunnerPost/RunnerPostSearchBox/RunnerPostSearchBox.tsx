...
import TagIcon from '../../../asset/tag-icon.svg';
...
interface Props {
  ...
  setReviewStatus: React.Dispatch<
    React.SetStateAction<ReviewStatus | undefined>
  >;
  setEnteredTag: React.Dispatch<React.SetStateAction<string>>;
}

const RunnerPostSearchBox = async ({
  reviewStatus,
  setReviewStatus,
  setEnteredTag,
}: Props) => {
  ...
  const searchedTags = (await getSearchTag(keywordForResponse)).data;
  ...
  const handleArrowUp = () => {
    ...
    setKeyword(
      nextIndex === 0 ? inputBuffer : searchedTags[nextIndex - 1].tagName
    );
  ...
  return (
    <S.SearchBoxContainer onSubmit={handleSubmit}>
      ...
      >
        ...
        />
        <S.SearchedTagList
          $isVisible={searchedTags.length > 0 && isInputFocused}
        >

...
const Calendar = (props: CalendarProps) => {
	...
	const { monthInfo, setPrevMonth, setNextMonth } = useCalendar(
	...
	const daysOfWeeks = DAYS_OF_THE_WEEK.map((day) => (
		<DaySmallBox key={day} date={day} />
	));
	...
	const days = Array.from({ length: boxLength }).map((_, idx) => {
		...
		const clickHandler = () =>
			isInRange
				...
				: setMessage(
						`${convertDateKorYear(currentDate)}은 범위 내 날짜가 아닙니다`
				  );
		...
		return isShow ? (
			<DaySmallBox
				...
				isSelected={
					selectedDate !== null &&
					getDaysBetween(currentDate, selectedDate) === 0
				}

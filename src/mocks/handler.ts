...
export const handlers = [
  ...
  rest.post("/admin/cafes", async (req, res, ctx) => {
    const { businessRegistrationNumber, name, roadAddress, detailAddress } =
      await req.json();
  ...
  rest.post("/admin/temporary-customers", async (req, res, ctx) => {
    ...
    return res(
      ctx.status(201),
      ctx.set("Location", `customers/${createdCustomer.id}`)
    );
  }),
  ...
  rest.get("/admin/customers/:customerId/coupons", (req, res, ctx) => {
    ...
    const findCouponsResult = coupons.find(
      (coupon) =>
        coupon.customerId === +customerId && activeQueryParam === "true"
    );
  ...
  // 스탬프 적립
  rest.post(
    "/admin/customers/:customerId/coupons/:couponId/stamps",
    async (req, res, ctx) => {
  ...
  // 리워드 사용
  rest.patch(
    "/admin/customers/:customerId/rewards/:rewardId",
    async (req, res, ctx) => {
  ...
  rest.get("/profiles", (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json({
        profile: { id: 10, nickname: "강영민", phoneNumber: null, email: null },
      })
    );
  }),
  ...
  rest.get("/customers/profiles/search", (req, res, ctx) => {
    ...
    if (phoneNumber === "01011111111")
      return res(
        ctx.status(200),
        ctx.json({ customers: [customerRegisterType[0]] })
      );

    if (phoneNumber === "01022222222")
      return res(
        ctx.status(200),
        ctx.json({ customers: [customerRegisterType[1]] })
      );

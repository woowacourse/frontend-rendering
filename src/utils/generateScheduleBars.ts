...
import { CALENDAR, ONE_DAY_MS } from '@/constants/calendar';
...
const getFirstLastDateOfCalendar = (year: number, month: number) => {
  ...
  const firstDateOfCalendar = new Date(
    firstDateOfMonth.getTime() - ONE_DAY_MS * firstDateOfMonth.getDay(),
  );
  const lastDateOfCalendar = new Date(
    firstDateOfCalendar.getTime() +
      CALENDAR.ROW_SIZE * CALENDAR.COLUMN_SIZE * ONE_DAY_MS -
      1,
  );
  ...
  Array.from(
    ...
    (_, index) => {
      const currentDate = new Date(
        firstDateOfCalendar.getTime() + index * ONE_DAY_MS,
      );
...
const calcDuration = (start: Date, end: Date) => {
  ...
  return Math.abs(diff / ONE_DAY_MS) + 1;
};

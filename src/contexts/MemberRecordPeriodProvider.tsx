...
const MemberRecordPeriodContext =
  createContext<MemberRecordPeriodContextType | null>(null);
...
const MemberRecordPeriodProvider = ({ children }: PropsWithChildren) => {
  ...
  const updatePeriod = (period: Period) => {
    ...
    if (period === 'week')
      newStartDate = format.date(new Date(today.setDate(day - 7)), '-');

    if (period === 'oneMonth')
      newStartDate = format.date(new Date(today.setMonth(month - 1)), '-');

    if (period === 'threeMonth')
      newStartDate = format.date(new Date(today.setMonth(month - 3)), '-');
  ...
  const updateStartEndDate = (date: Date) => {
    ...
    if (
      searchParams.start &&
      !searchParams.end &&
      new Date(searchParams.start) > date
    ) {
  ...
  const isIncludeSelectDate = (date: Date) => {
    ...
    if (
      new Date(searchParams.start) < date &&
      new Date(searchParams.end) >= date
    )
      return true;
  ...
  const value = {
    ...
    triggerSearchRecord,
    isMiddleSelectedCustomDate: (date: Date) =>
      isSoonSelectedDate(date) || isIncludeSelectDate(date),
    updatePeriod,
    updateStartEndDate,
    updateHoverDays,
    updatePage,
  ...
  return (
    <MemberRecordPeriodContext.Provider value={value}>
      {children}
    </MemberRecordPeriodContext.Provider>
  );
};

...
export const useCityTags = (initialCityTags: CityData[]) => {
  ...
  const addCityTag = (selectedCity: CityData) => {
    ...
    setCityTags((prevCityTags) => {
      const hasCityTag = !!prevCityTags.filter(
        (tags) => tags.id === selectedCity.id
      ).length;

      if (hasCityTag) {
        const filteredCityTags = prevCityTags.filter(
          (cityTag) => cityTag.id !== selectedCity.id
        );
        return [...filteredCityTags, { ...selectedCity, name: cityName }];
      }

      if (prevCityTags.length >= 15) {
        return prevCityTags;
      }
  ...
  const deleteCityTag = (selectedCity: CityData) => {
    setCityTags((prevCityTags) =>
      prevCityTags.filter((cityTag) => cityTag.id !== selectedCity.id)
    );
  };

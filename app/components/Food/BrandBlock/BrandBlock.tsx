...
const BrandBlock = (brandBlockProps: BrandBlockProps) => {
  ...
  const [onResearchCenterTip, setOnResearchCenterTip] =
    useState<boolean>(false);

  const onToolTip = (
    toolTipType: 'foundedYear' | 'state' | 'vet' | 'reserachCenter'
  ) => {
    ...
    setOnResearchCenterTip(
      toolTipType === 'reserachCenter' && !onResearchCenterTip
    );
  };
    ...
    <BrandBlockWrapper>
      <BrandProfile>
        ...
        <BrandInfo>
          ...
          <BrandFoundedYearContainer onClick={() => onToolTip('foundedYear')}>
            <BrandFoundedYear>{`창립연도 ${foundedYear}년`}</BrandFoundedYear>
          </BrandFoundedYearContainer>
        </BrandInfo>
      </BrandProfile>
      <BrandPersonalityList>
        <BrandPersonality onClick={() => onToolTip('state')}>
          <BrandPersonalityImg
...
const BrandFoundedYearContainer = styled.div`
  position: relative;
...
const BrandPersonality = styled.div`
  position: relative;

...
export const isDateFormat = (target: string): target is DateFormat => {
	...
	if (date < new Date(1900, 1, 1) || date > new Date(2099, 12, 31))
		return false;
 ...
 */
export const isKoreanDateFormat = (
	target: string
): target is KoreanDateFormat => {
 ...
 */
export const convertDateKorYear = (
...
export const getDaysBetween = (
	...
	another: string | number | Date
) => {
	const first =
		typeof one === 'string' && isDateFormat(one)
			? getStringToDate(one)
			: new Date(one);
	const second =
...
export const getParticularDateFromSpecificDay = (
	...
	const particularDate = new Date(
		specificDay.setDate(specificDay.getDate() + particularNumber)
	);
...
export const getDayInfo = ({ idx, monthInfo, min, max }: DayInfo) => {
	...
	const currentDate = getStringToDate(
		`${monthInfo.year}-${monthInfo.month}-${date}`
	);
	...
	const isInRange = DateValidate.isDateInRange({

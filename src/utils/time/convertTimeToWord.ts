...
export const convertTimeToWord = (
  date: StringDate,
  ...
  const timeDifference = Math.floor(
    (targetDate.getTime() - currentDate.getTime()) / 60000
  );
  ...
  if (Math.round(positiveTimeDifference / (time.hour * time.minute)) > 0)
    return `${Math.round(
      positiveTimeDifference / (time.hour * time.minute)
    )}일 ${afterBefore}`;

  if (Math.round(positiveTimeDifference / time.minute) > 0)
    return `${Math.round(
      positiveTimeDifference / time.minute
    )}시간 ${afterBefore}`;
